...
Este capítulo se enfoca en explorar las técnicas empleadas por los atacantes para explotar las vulnerabilidades inherentes de las redes OPN. Desde eludir portales cautivos hasta la suplantación de direcciones MAC e IP, y la creación de puntos de acceso falsos. Comprender estos métodos es crucial tanto para la defensa como para la ofensiva en el ámbito de la seguridad informática.
...
```
>[!NOTE]
>Reemplace INTERFAZ con su interfaz Wi-Fi (por ejemplo, wlan0, wlan1) y CLIENTE_MAC con la dirección MAC de un cliente ya autenticado en el portal cautivo. Estos comandos se tienen que ejecutar como root
...
```
Y nos conectamos a la red con wpa_supplicant.
```
...
- /IP_cliente_autorizado/: La dirección IP del cliente autenticado (víctima).
- /IP_gateway/ La dirección IP del gateway (puerta de enlace).
...
- -d ! LAN: Aplica la regla a todos los destinos que no están en la red local ( lan).
- -j SNAT --to IP_cliente_autorizado: Realiza Source NAT (SNAT), cambiando la dirección de origen a la IP del cliente autorizado.
...
- -d <IP_cliente_autorizado>: Especifica que la regla afecta al tráfico destinado a la IP del cliente autenticado.
...
sudo airodump-ng INTERFAZ_MON -w ~/wifi/scan --manufacturer --wps -c <CHANNEL>
```
...
![](https://files.cdn.thinkific.com/file_uploads/937577/images/0fe/d61/895/1724017976219.png?width=1920)

Tráfico sin cifrar HTTP en red OPN
...
```
![Ejecutando eaphammer con opciones de karma attack](https://files.cdn.thinkific.com/file_uploads/937577/images/0e9/317/a72/1751197005878.png?width=1920)

Lanzando eaphammer con la opción --karma para responder a todas los Probe Requests
...
![Lista de redes preferidas del cliente mostrando open-wifi y WiFi-Restaurant](https://files.cdn.thinkific.com/file_uploads/937577/images/723/ac3/e15/1751197020587.png?width=1920)

La Preferred Network List del cliente incluye "open-wifi" y "WiFi-Restaurant", ambas redes abiertas

Luego desautenticamos al cliente para forzarlo a desconectarse de wifi-global:

![Desautenticando al cliente de wifi-global](https://files.cdn.thinkific.com/file_uploads/937577/images/736/649/b20/1751197026450.png?width=1920)

Enviando tramas de desautenticación para desconectar al cliente de "wifi-global"

Inmediatamente después, el cliente se conecta al falso AP:

![Cliente conectado al punto de acceso malicioso](https://files.cdn.thinkific.com/file_uploads/937577/images/344/df9/33b/1751197032000.png?width=1920)

El cliente se ha asociado al punto de acceso malicioso

En la siguiente imagen, vemos el falso AP emitiendo el WiFi-Restaurant SSID:

![AP malicioso emitiendo SSID WiFi-Restaurant](https://files.cdn.thinkific.com/file_uploads/937577/images/d7e/414/ff2/1751197037885.png?width=1920)

El AP malicioso está emulando el SSID "WiFi-Restaurant" de la PNL del cliente

También podemos ver que eaphammer intenta recrear todos los ESSIDs encontrados en las solicitudes de sondeo del cliente:

![eaphammer creando ESSIDs a partir de las solicitudes de sondeo](https://files.cdn.thinkific.com/file_uploads/937577/images/213/cca/fc1/1751197043376.png?width=1920)

eaphammer generando múltiples SSIDs basados en las solicitudes de sondeo del cliente
