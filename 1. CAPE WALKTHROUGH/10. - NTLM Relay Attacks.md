...
![NTLM_Relay_Attacks_Walkthrough_Image_1.png](https://academy.hackthebox.com/storage/walkthroughs/122/NTLM_Relay_Attacks_Walkthrough_Image_1.png)
...
Once it receives the `AUTHENTICATE_MESSAGE`, and because it does not possess the client's secret key, the server delegates the verification of the user's identity to a DC (a procedure known as Pass-through authentication) by invoking `NetrLogonSamLogonWithFlags`, which contains `NETLOGON_NETWORK_INFO`.

![NTLM_Relay_Attacks_Walkthrough_Image_2.png](https://academy.hackthebox.com/storage/walkthroughs/122/NTLM_Relay_Attacks_Walkthrough_Image_2.png)
...
![NTLM_Relay_Attacks_Walkthrough_Image_3.png](https://academy.hackthebox.com/storage/walkthroughs/122/NTLM_Relay_Attacks_Walkthrough_Image_3.png)
...
![NTLM_Relay_Attacks_Walkthrough_Image_4.png](https://academy.hackthebox.com/storage/walkthroughs/122/NTLM_Relay_Attacks_Walkthrough_Image_4.png)
...
![NTLM_Relay_Attacks_Walkthrough_Image_5.png](https://academy.hackthebox.com/storage/walkthroughs/122/NTLM_Relay_Attacks_Walkthrough_Image_5.png)
...
Once the students have started the tool in analyze mode, they will start capturing the broadcasted traffic without interacting (poisoning) with it. After a while, students will find out that the traffic originating for `172.16.117.50` is coming from `WORKSTATION01`.
...
Once students have confirmed that `RMONTY`'s authentication has been relayed successfully, they will use `proxychains` and `smbclient.py`. They are able to gain SMB access on the `172.16.117.50` host, using the `-no-pass` option in `smbclient.py` the relayed authentication will be used to access the resources on the mentioned target.
...
Right after the students have successfully escalated the privileges of the computer account, they will have to utilize `secretsdump.py` part of Impacket to perform a `DCSync` attack and dump the hashes of the domain users, including the user `rons`.
...
### "Use Coercer in 'coerce' mode against 172.16.117.60 and submit the name of the first RPC call resulting in the message '[+] (ERROR_BAD_NETPATH)' for the SMB named pipe '\PIPE\lsass'."
...
Subsequently, students will come to know that the coerced authentication was successful and relayed, and the attack has succeeded as `Academy-Student$` can now impersonate users on `SQL01$` via `S4U2Proxy`.
...
Once students have executed the command successfully an Administrator ticket will be generated, which can be used to access resources via `psexec` and its `-k` option to use Kerberos, the `-no-pass` option to specify that no password will be used on the `SQL01` machine. And, the `KRB5CCNAME` environment variable to specify the Administrator's ccache file.
...
Subsequently, once students have initiated the coerce authentication through `PetitPotam`, they will come to know that a certificate was generated in `ntlmrelayx.py`:
