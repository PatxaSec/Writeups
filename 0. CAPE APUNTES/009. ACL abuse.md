...
En términos de limpieza, hay algunas cosas que tenemos que hacer:

1. Quitar el falso SPN que creamos en el usuario `adunn` .
```powershell
Set-DomainObject -Credential $Cred2 -Identity adunn -Clear serviceprincipalname -Verbose
```
2. Quitar el `damundsen`usuario del grupo `Help Desk Level 1`
3. Establezca la contraseña para el usuario`damundsen` de nuevo a su valor original (si lo conocemos) o tiene nuestro cliente configurarlo/alerta del usuario
```powershell
Remove-DomainGroupMember -Identity "Help Desk Level 1" -Members 'damundsen' -Credential $Cred2 -Verbose
```

#### Confirmando damundsen fue retirado del Grupo

```powershell
Get-DomainGroupMember -Identity "Help Desk Level 1" | Select MemberName |? {$_.MemberName -eq 'damundsen'} -Verbose
```

Este orden es importante porque si eliminamos al usuario del grupo primero, entonces no tendremos los derechos para eliminar el falso SPN.

Primero, quitemos el falso SPN de la cuenta  `adunn`.



## Mitigación

## Detección y desmeditación

Algunas recomendaciones en torno a los ACLs incluyen:

1. `Auditing for and removing dangerous ACLs`

Las organizaciones deben tener auditorías regulares de AD realizadas, pero también entrenar al personal interno para ejecutar herramientas como BloodHound e identificar ACLs potencialmente peligrosos que se pueden eliminar.

2. `Monitor group membership`

La visibilidad en grupos importantes es primordial. Todos los grupos de alto impacto del dominio deben ser monitoreados para alertar al personal de TI de los cambios que podrían ser indicativos de una cadena de ataque ACL.

3. `Audit and monitor for ACL changes`

Habilitar la [Política de Auditoría de Seguridad Avanzada](https://docs.microsoft.com/en-us/archive/blogs/canitpro/step-by-step-enabling-advanced-security-audit-policy-via-ds-access) puede ayudar a detectar cambios no deseados, especialmente [Event ID 5136: Se modificó un objeto de servicio de directorios](https://docs.microsoft.com/en-us/windows/security/threat-protection/auditing/event-5136) que indicaría que el objeto de dominio fue modificado, lo que podría ser indicativo de un ataque de ACL. Si nos fijamos en el registro del evento después de modificar el ACL del objeto de dominio, veremos algún ID de evento `5136`creados
...
![image](../Imágenes/20250619123657.png)

![image](../Imágenes/20250619123716.png)

![image](../Imágenes/20250619123730.png)

![image](../Imágenes/20250619124526.png)
